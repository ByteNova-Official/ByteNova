...
def generate():
    ...
    if response.status_code >= 300:
        return {'result': f'Upload failed with status code {response.status_code}, response: {response.text}'}
    else:
        return {'result': "https://cdn.clustro.ai/" + s3_upload_fields['fields']['key']}
